...
class IhsCameraSurface extends StatefulWidget {
  const IhsCameraSurface({
    super.key,
  ...
  @override
  State<IhsCameraSurface> createState() => _IhsCameraSurfaceState();
}

class _IhsCameraSurfaceState extends State<IhsCameraSurface> {
  _IhsCameraController? _controller;
  String? _error;

  void _reportError(Object error) {
    debugPrint('Argo camera: IHS surface unavailable: $error');
  ...
  Widget build(BuildContext context) {
    if (_error != null) {
      return const Center(child: Text('Native camera surface unavailable'));
    }
    return LayoutBuilder(
      builder: (context, constraints) {
        if (!constraints.hasBoundedWidth || !constraints.hasBoundedHeight) {
          return const Text('Native camera requires bounded dimensions');
        }
        ...
        return PlatformViewLink(
          ...
          onCreatePlatformView: (params) {
            return _controller = _IhsCameraController(
              viewId: params.id,
              viewType: params.viewType,
              creationParams: {'role': widget.role.index},
              direction: direction,
...
class _IhsCameraController extends PlatformViewController {
  _IhsCameraController({
  ...
  Future<void> create({Size? size, Offset? position}) {
    ...
    return _enqueue(() async {
      if (_disposed) return;
      final encoded = ByteData(12)
        ..setUint32(0, 0x41524356) // ARCV
        ..setUint32(4, 1, Endian.little)
        ..setUint32(8, creationParams['role']! as int, Endian.little);
      _createIssued = true;
      ...
      if (diagnostics) {
        debugPrint(
          'Argo camera: IHS create acknowledged id=$viewId; '
  ...
  // CameraPage's outer Listener already maps all camera input. Never
